...
import { plazo, porcentaje, usd } from '@/utils/formato';

import DonutPortafolio, { COLORES } from '../components/DonutPortafolio';
import { getPropuesta } from '../services/investorApi';
import type { AssetAllocation, PortfolioProposal } from '../types/inversionista';

type Props = NativeStackScreenProps<InvestorStackParamList, 'Propuesta'>;
  ...
  return (
    <View className="gap-3 rounded-2xl border border-surface-border bg-surface-background p-5">
      ...
      {linea.retorno_esperado != null ? (
        <Text className="text-caption text-text-muted">
          Retorno estimado referencial: {porcentaje(linea.retorno_esperado)} anual. No es
 ...
 *
 * La primera visita **genera** la propuesta en el backend: ahí es donde corre el LLM y
 * su guardarraíl, así que la espera puede ser de varios segundos. Si Gemini falla, el
 * backend igual devuelve la explicación determinista — esta pantalla nunca se queda sin
 * texto, y nunca muestra un número que el LLM haya inventado.
 */
export default function PropuestaPage({ navigation }: Props) {
  const { user } = useAuth();
  ...
  const cargar = useCallback(async () => {
    ...
    setPropuesta(null);
    try {
      setPropuesta(await getPropuesta(user.id));
    } catch (e) {
  ...
  return (
    <SafeAreaView className="flex-1 bg-surface-background">
      ...
      <View className="flex-row items-center gap-3 border-b border-surface-border px-5 py-4">
        ...
        <Text className="text-heading font-bold text-text-primary">Tu propuesta</Text>
      </View>
      ...
      >
        ...
        <TouchableOpacity
          onPress={() => navigation.navigate('ComoSeCalculo', { sessionId: propuesta.session_id })}
          activeOpacity={0.85}
